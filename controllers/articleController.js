...
module.exports = {
  ...
  listAllSubCategories: async (req, res, next) => {
    try {
      ...
      //   let articles = await Articles.find({categories: {$in:[subCategory._id]}})
      //   subCategory.count = articles.length
      // }

      for (i = 0; i < subCategories.length; i++) {
        let articles = await Articles.find({
          categories: subCategories[i]._id + "",
        });
        subCategories[i].count = articles.length;
      }

      // subCategories.map(async(e,i) => {
      //   let articles = await Articles.find({categories: {$in:[e._id]}})
      //   console.log('articles found:',articles)
      //   e.count = articles.length
      // })
