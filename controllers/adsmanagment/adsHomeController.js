...
function checkHomeTopCatMainCatAd1Ad6Ad8SliderType(topCatMainCatAd1Ad6Ad8SliderTypes, fileInfo) {
    ...
    if (topCatMainCatAd1Ad6Ad8SliderTypes == "ad1") {
        if (dimensions.width != 193 || dimensions.height != 132) {
            imageError = "Please upload image of size 193 * 132";
        }
...
module.exports = {
    ...
    getAdsHomeCart: async (req, res, next) => {
        try { 
            let result = await AdsHomeCart.find({isDisabled:false}).populate({
                path: "subCategoryId",
                select: ["_id", "title", "image"],
                       })
                       if(result.length){
                     for(i=0;i<result.length;i++){
                            result[i].subCategoryId.image= process.env.BASE_URL.concat(result[i].subCategoryId.image)
                     }
                    }
                res.status(200).json({
                    status: false,
