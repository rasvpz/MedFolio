...
module.exports = {
    ...
    editVideo:async(req,res,next)=>{
        try {
            let existingVideo = await FolifitFitnessClub.findOne({_id:req.params.id})
            if(existingVideo){
                let data = {
                    title:req.body.title,
                    descriptionEnglish:req.body.descriptionEnglish,
                    ...
                    if(req.files.video){
                        ...
                        await client.upload(
                            ...
                            function(uri) {
                                ...
                                let splittedUri = uri.split('/')
                                data.video = splittedUri[2]
                                FolifitFitnessClub.updateOne({_id:req.params.id},data).then((response)=>{
    ...
    listFitnessClubVideo:async(req,res,next)=>{
     ...
     listPopularFitnessClubVideo:async(req,res,next)=>{
        try {
            let data = await Read.aggregate([{ $match:{type: "fitnessClub" }},{
                $project:{
                  _id: 1,
                  type: 1,
                  contentId: 1,
            ...
            for(single of data) {
              let video = await FolifitFitnessClub.findOne({_id:mongoose.Types.ObjectId(single.contentId)},{
                title:1,
                video:1,
                thumbnail:{ $concat: [ imgPath,"$thumbnail" ] },
                gif:1
    ...
    getFitnessClubById:async(req,res,next)=>{
        try {
            let result = await FolifitFitnessClub.findOne({_id:req.params.id}).lean()
    ...
    getFitnessSubCatByMain:async(req,res,next)=>{
        try {
            if(req.params.type=='weeklyworkout'){
                ...
                let result = await FoliofitMasterFitnessMainHomeFullbodyHealthy.find({fitnessType:req.params.type},{title:1}).lean()
                res.status(200).json({
                    status: true,
                    data:result,
                });
            ...
            if(req.params.type=='weeklyworkout'){
                ...
                for(let item of result){
                    for(let ids of item.videos){
                        let video = await FolifitFitnessClub.findOne({_id:mongoose.Types.ObjectId(ids)},{
                            title:1,
                            video:1,
                            thumbnail:{ $concat: [ imgPath,"$thumbnail" ] },
                            gif:1
                ...
                let result = await FoliofitMasterFitnessMainHomeFullbodyHealthy.find({fitnessType:req.params.type}).lean()
                let videos = []
                for(let item of result){
                    for(let ids of item.videos){
                        let video = await FolifitFitnessClub.findOne({_id:mongoose.Types.ObjectId(ids)},{
    ...
    deleteVideo:async(req,res,next)=>{
        try {
            ...
            await FolifitFitnessClub.deleteOne({_id:req.params.id}).then((response)=>{
                res.status(200).json({
                    status: true,
    ...
    getFitnessClubByCatId:async(req,res,next)=>{
        try {
            let data = []
            let result 
            let resultMain = await FoliofitMasterFitnessMainHomeFullbodyHealthy.findById(req.params.id).lean()
            if(resultMain) result = resultMain
           let resultOther= await foliofitWeeklyWorkout.findById(req.params.id).lean()
           if(resultOther) result = resultOther
           for(let id of result.videos){
            let video = await FolifitFitnessClub.findOne({_id:id},{
                title:1,
                video:1,
                thumbnail:{ $concat: [ imgPath,"$thumbnail" ] },
                gif:1
